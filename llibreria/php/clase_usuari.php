...
//,id_prestec  ,'$this->$id_prestec'
   ...
   //NOOOO $sql = "DELETE FROM usuari where id = (".$this->id.")";
    ...
    if (!$conexion){//Comprobo que podem establir conexió sino mostro error
     ...
     var_dump($this->id);
    $sql ="UPDATE usuari set nom ='$this->nom', dni ='$this->dni', adreca ='$this->adreca', poblacio='$this->poblacio', provincia ='$this->provincia', nacio_pais='$this->nacio_pais', data_naixement='$this->data_naixement', email ='$this->email', telefon='$this->telefon',  cognom ='$this->cognom' where id =  $this->$row[0])";
